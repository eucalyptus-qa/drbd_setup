...
class DRBDQAConfigurator(object):
    ...
    def get_hosts_from_ips(self):
        for ip in self.ips:
            cmd = ['ssh', 'root@%s' % ip, 'sh -c \'if ( ! hostname | grep eucalyptus ); then hostname `hostname`.eucalyptus; fi\'']
            temp_string = Popen(cmd, stdout=PIPE, stderr=PIPE).communicate()
    ...
    def zero_dev(self):
        for ip in self.ips:
            cmd = ['ssh', 'root@%s' % ip, 'dd if=/dev/zero of=/dev/sdb bs=1M count=10']
            print Popen(cmd, stdout=PIPE, stderr=PIPE).communicate()
